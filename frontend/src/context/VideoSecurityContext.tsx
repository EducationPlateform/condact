...
import { securityService, SecurityViolation } from '../services/securityService';

interface VideoSecurityState {
  isSecureMode: boolean;
  violations: SecurityViolation[];
  reportViolation: (type: string, details: any) => Promise<void>;
  getViolations: (params?: { studentId?: string; lectureId?: string; videoId?: string }) => Promise<void>;
}

const VideoSecurityContext = createContext<VideoSecurityState | undefined>(undefined);

export const VideoSecurityProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const getViolations = useCallback(async (params?: { studentId?: string; lectureId?: string; videoId?: string }) => {
    try {
  ...
  const value: VideoSecurityState = {
    ...
    reportViolation: async (type: string, details: any) => {
      ...
      throw new Error('reportViolation must be called with lectureId and videoId');
    },
    getViolations,
  };
...
export const useVideoSecurity = () => {
  ...
  if (context === undefined) {
    throw new Error('useVideoSecurity must be used within a VideoSecurityProvider');
  }
  return context;
};
