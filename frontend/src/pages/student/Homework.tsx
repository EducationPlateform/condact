...
const StudentHomework: React.FC = () => {
  const [homeworks, setHomeworks] = useState<{ homework: Homework; lecture: Lecture }[]>([]);
        ...
        for (const group of groups) {
          try {
            ...
            for (const lecture of lectures) {
              try {
                const homework = await homeworkService.getByLecture(lecture._id);
            ...
            <Card>
              <CardContent>
                ...
                <Typography variant="body2" sx={{ mt: 1 }}>
                  {item.homework.questions.length} questions • Max Score: {item.homework.maxScore}
                </Typography>
              </CardContent>
              <CardActions>
                <Button
